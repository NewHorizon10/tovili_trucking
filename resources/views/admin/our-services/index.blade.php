...
<div class="content  d-flex flex-column flex-column-fluid" id="kt_content">
    ...
    <div class="d-flex flex-column-fluid">
        <div class=" container ">
            <form action="{{route('our-services.index')}}" method="get" class="kt-form kt-form--fit mb-0" autocomplete="off">
                        ...
                        <div class="card card-custom card-stretch card-shadowless">
                            <div class="card-header">
                                ...
                                <div class="card-toolbar">
                                    ...
                                    <a href="{{route('our-services.create')}}" class="btn btn-primary">{{trans("messages.admin_common_Add_New")}} {{trans("messages.admin_common_Our_Service")}} </a>
                                </div>
                            </div>
                            <div class="card-body">
                                    ...
                                    <div id="collapseOne6" class="collapse <?php echo !empty($searchVariable) ? 'show' : ''; ?>" data-parent="#accordionExample6">
                                        <div>
                                            <div class="row mb-6">
                                                ...
                                                <div class="col-lg-3 mb-lg-5 mb-6">
                                                    <label>{{trans("messages.admin_common_Title")}}</label>
                                                    <input type="text" class="form-control" name="title" placeholder="{{trans("messages.admin_common_Title")}}" value="{{$searchVariable['title'] ?? '' }}">
                                                </div>
                                            ...
                                            <div class="row mt-8">
                                                <div class="col-lg-12">
                                                    ...
                                                    &nbsp;&nbsp;
                                                    <a href='{{ route("our-services.index")}}' class="btn btn-secondary btn-secondary--icon">
                                                        <span>
                                        ...
                                        <table class="table dataTable table-head-custom table-head-bg table-borderless table-vertical-center" id="taskTable">
                                            <thead>
                                                <tr class="text-uppercase">
                                                   <th> {{trans("messages.admin_common_Image")}} </th>

                                                
                                                    <th class="{{(($sortBy == 'title' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'title' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route('our-services.index',array(	'sortBy' => 'title',
													'order' => ($sortBy == 'title' && $order == 'desc') ? 'asc' : 'desc',
													$query_string))}}"> {{trans("messages.admin_common_Title")}}</a>
                                                    </th>

                                                    <th class="{{(($sortBy == 'title' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'title' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route('our-services.index',array(	'sortBy' => 'title',
													'order' => ($sortBy == 'title' && $order == 'desc') ? 'asc' : 'desc',
													$query_string))}}">{{trans("messages.admin_common_button_text")}}</a>
                                                    </th>
                                                    <th class="{{(($sortBy == 'title' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'title' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route('our-services.index',array(	'sortBy' => 'title',
													'order' => ($sortBy == 'title' && $order == 'desc') ? 'asc' : 'desc',
													$query_string))}}">{{trans("messages.admin_common_button_link")}}</a>
                                                    </th>
                                                    <th class="{{(($sortBy == 'description' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'description' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route('our-services.index',array(	'sortBy' => 'description',
                                                    ...
                                                    <th class="{{(($sortBy == 'created_at' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'created_at' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route('our-services.index',array(	'sortBy' => 'created_at',
													'order' => ($sortBy == 'created_at' && $order == 'desc') ? 'asc' : 'desc',
													$query_string))}}">{{trans("messages.admin_common_Added_On")}}</a>
                                                    </th>
                                                    <th class="{{(($sortBy == 'is_active' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'is_active' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route('our-services.index',array(	'sortBy' => 'is_active',
                                            ...
                                            <tbody>
                                                ...
                                                @if(!empty($result->image))
                                                        ...
                                                        <a class="fancybox-buttons" data-fancybox-group="button" href="{{asset( Config('constants.OURSERVICE_ROOT_PATH').$result->image) }}">
                                                            <img height="50" width="50" src="{{asset( Config('constants.OURSERVICE_PATH').$result->image) }}" />
                                                        </a>															</div>
													   @endif
                                                    </td>
                                                    ...
                                                    <td>
                                                        <div class="text-dark-75 mb-1 font-size-lg" title="{{ $result->description }}">
                                                            {{ Illuminate\Support\Str::limit($result->description, 20) . (str_word_count($result->description) > 20 ? '...' : '') }}
                                                        </div>
                                                    </td>
                                                    ...
                                                    @if($result->is_active == 1)
                                                    <a title="{{trans('messages.admin_common_Click_To_Deactivate')}}" href='{{route("our-services.status",array(base64_encode($result->id),0))}}' class="btn btn-icon btn-light btn-hover-danger btn-sm status_any_item" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" data-original-title="Click To Deactivate">
                                                    ...
                                                    @else
                                                    <a title="{{trans('messages.admin_common_Click_To_Activate')}}" href='{{route("our-services.status",array(base64_encode($result->id),1))}}' class="btn btn-icon btn-light btn-hover-success btn-sm status_any_item" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" data-original-title="Click To Activate">
                                                    ...
                                                    @endif
                                                    <a href="{{route('our-services.edit',base64_encode($result->id))}}" class="btn btn-icon btn-light btn-hover-primary btn-sm" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" title="" data-original-title="{{trans('messages.admin_common_Edit')}}">
                                                    ...
                                                    <a href="{{route('our-services.delete',base64_encode($result->id))}}" class="btn btn-icon btn-light btn-hover-danger btn-sm confirmDelete" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" title="" data-original-title="{{trans('messages.admin_common_Delete')}}">
                                                        <span class="svg-icon svg-icon-md svg-icon-danger">
                                                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                                                <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                                                    <rect x="0" y="0" width="24" height="24" />
