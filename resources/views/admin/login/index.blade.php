                ...
                <div class="login-form">
                    <form action="{{route('adminpnlx')}}" method="post" class="form" id="kt_login_singin_form">
                        ...
                        <div class="mb-12 text-center">
                            <h3 class="font-weight-bold text-dark">{{trans('messages.admin_common_Sign_In')}}</h3>
                            <p>{{trans('messages.admin_common_Enter_your_details_to_login_to_your_account')}}:</p>
                        </div>
                        ...
                        <div class="form-group">
                            <div class="d-flex justify-content-between mt-n5">
                                <label class="font-size-h6 font-weight-bolder text-dark pt-5">{{trans('messages.admin_common_Your_Password')}}</label>
                            ...
                            <input type="password" name="password" placeholder="{{trans('messages.Password')}}" class="form-control form-control-solid h-auto py-5 px-6 rounded-lg border-0 @error('password') is-invalid @enderror" value=""  autocomplete="false" ]>
                            @if ($errors->has('password'))
                            <div class="invalid-feedback">{{ $errors->first('password') }}</div>
                            @endif
                        </div>
                        <div class="text-center">
                            <input type="submit" value="{{trans('messages.login')}}" class="btn btn-primary font-weight-bolder font-size-h6 px-12 py-4 my-3">
                        </div>
                    </form>
                </div>
            </div>
