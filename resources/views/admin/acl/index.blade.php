			...
			<div class="d-flex align-items-center flex-wrap mr-1">
				<div class="d-flex align-items-baseline flex-wrap mr-5">
					<h5 class="text-dark font-weight-bold my-1 mr-5">
						{{Config('constants.ACL.ACLS_TITLE')}}
					</h5>
						...
						<div class="card card-custom card-stretch card-shadowless">
							<div class="card-header">
								...
								<div class="card-toolbar">
									...
									<a href='{{route("$model.create")}}' class="btn btn-primary"> {{ trans("Add New") }} {{Config('constants.ACL.ACL_TITLE')}} </a>
								</div>
							</div>
							<div class="card-body">
										...
										<table class="table dataTable table-head-custom table-head-bg table-borderless table-vertical-center" id="taskTable">
											<thead>
												<tr class="text-uppercase">
													<th class="{{(($sortBy == 'parent_id' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'parent_id' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
														<a href="{{route($model.'.index',array(	'sortBy' => 'parent_id',
													...
													<th class="{{(($sortBy == 'module_order' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'module_order' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
														<a href="{{route($model.'.index',array(	'sortBy' => 'module_order',
											...
											<tbody>
												...
												<tr>
													<td>
														...
														{{AclparentByName($result->parent_id)}}
														</div>
													</td>
													<td>
														<div class="text-dark-75 mb-1 font-size-lg">
													...
													<td class="text-right pr-2">
														@if($result->is_active == 1)
														<a title="Click To Deactivate" href='{{route("$model.status",array($result->id,0))}}' class="btn btn-icon btn-light btn-hover-danger btn-sm status_any_item" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" data-original-title="Click To Deactivate">
														...
														@else
														<a title="Click To Activate" href='{{route("$model.status",array($result->id,1))}}' class="btn btn-icon btn-light btn-hover-success btn-sm status_any_item" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" data-original-title="Click To Deactivate">
