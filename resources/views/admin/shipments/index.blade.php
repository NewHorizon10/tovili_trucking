                                            ...
                                            <div class="row mb-6">
                                                ...
                                                <div class="col-lg-3  mb-lg-5 mb-6">
                                                    ...
                                                    <select name = "status" class="form-control select2init">
                                                        <option value="">{{trans('messages.Select')}} {{trans('messages.admin_common_Status')}}</option>
                                                        <option value = "paid" {{ isset($searchVariable['status']) && $searchVariable['status'] == 'paid' ? 'selected': '' }}>{{trans("messages.paid")}}</option>
                                                        <option value = "invoice" {{ isset($searchVariable['status']) && $searchVariable['status'] == 'invoice' ? 'selected': '' }}>{{trans("messages.admin_invoice")}}</option>
                                                        <option value = "cancelled" {{ isset($searchVariable['status']) && $searchVariable['status'] == 'cancelled' ? 'selected': '' }}>{{trans("messages.cancelled")}}</option>
                                                        <option value = "ended" {{ isset($searchVariable['status']) && $searchVariable['status'] == 'ended' ? 'selected': '' }}>{{trans("messages.end")}}</option>
                                                        <option value = "active" {{ isset($searchVariable['status']) && $searchVariable['status'] == 'active' ? 'selected': '' }}>{{trans("messages.active")}}</option>
                                                        <option value = "not_start" {{ isset($searchVariable['status']) && $searchVariable['status'] == 'not_start' ? 'selected': '' }}>{{trans("messages.not_start")}}</option>
                                                    </select>
                                                </div>
                                                <div class="col-lg-3 mb-lg-5 mb-6">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <a href="{{ route('shipment-request.show',$result->request_number) . '?tabs=offers' }}" class="text-dark-75">
                                                        {{ isset($result->approvedShipmentPrice->price) ? number_format($result->approvedShipmentPrice->price, 2) : (isset($result->rejectedShipmentOffersCount) && $result->rejectedShipmentOffersCount->count() > 0 ? $result->rejectedShipmentOffersCount->count() . ' offer'  : '--') }}
                                                         ...
                                                         @elseif($result->shipmentOffersCount ?->count() > 1)
                                                           <a href="{{ route('shipment-request.show',$result->request_number) . '?tabs=offers' }}" class="text-dark-75">  {{ $result->shipmentOffersCount ?->count() }} offers </a>
                                                           @elseif($result->shipmentOffersCount ?->count() == 1)
                                                           <a href="{{ route('shipment-request.show',$result->request_number) . '?tabs=offers' }}" class="text-dark-75">  {{ $result->shipmentOffersCount ?->count() }} offer </a>
                                                           @else 
                                                              --
                                                        @endif
                                                    ...
                                                    <td>
                                                        ...
                                                        @if($result->shipment_status && $result->status != "cancelled")
                                                            ...
                                                            @elseif($result->invoice_file != null && $result->payment_status == 'paid')
                                                                 {{ trans('messages.paid') }}
                                                            @elseif($result->invoice_file != null && $result->payment_status == 'unpaid')
                                                                 {{ trans('messages.admin_invoice') }}
                                                            @else
                                                    ...
                                                    <td class="text-right pr-2">
                                                        <a href="{{route('shipment-request.show',$result->request_number)}}" class="btn btn-icon btn-light btn-hover-primary btn-sm" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" title="" data-original-title="{{trans('messages.admin_common_View')}}">
