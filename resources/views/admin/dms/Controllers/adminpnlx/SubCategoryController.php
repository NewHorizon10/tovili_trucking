...
*/
	...
	public function list($id = ""){
		...
		/* seacrching on the basis of name and description */ 
			...
			return  View::make('admin.'.$this->model.'.index', compact('result','id','searchVariable','sortBy','order','query_string'));
		}
	...
	*/
	public function save($id=""){
		...
		if(!empty($formData)){
			...
			if ($validator->fails()){
				...
				$subCategory->category_name	        =	Input::get('category_name');
				$subCategory->category_description	=	Input::get('category_description');
				$subCategory->created_at		    =   date("Y-m-d H:i:s");
				$subCategory->save();
				// echo $data; die;
				Session::flash("success",trans("Sub Category added successfully."));
				return Redirect::to(`adminpnlx/itemCategory/subCategory/add/'$id'`);
			}
		}
	}
	...
	public function update($id=""){
		...
		if(!empty($thisData)){
			...
			if ($validator->fails()) {	
                ...
                return Redirect::to('adminpnlx/itemCategory/subCategory/',$id);
                // return Redirect::back();
			}
		}
	}
	...
	public function delete($id = ''){
		...
		if($id){	
			...
			Session::flash('flash_notice',trans("Sub Category has been deleted successfully.")); 
		}
		return Redirect::back();
	}
