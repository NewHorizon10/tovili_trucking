	...
	public function send_service_fifteen_day_reminders(){
		...
		if(!empty($retailServices)){
			foreach ($retailServices as $retailService) {
				...
				$message				=	"Your+DMS+vehicle+service+is+due+on+$serviceDate.";
				$ch = curl_init();
				curl_setopt($ch, CURLOPT_URL, "http://trans.inexus.in/API/SMSHttp.aspx?UserId=arunpayal7@gmail.com&pwd=pwd2019&Message=$message&Contacts=$phone_number&SenderId=DMS&ServiceName=SMSTRANS");
				...
				if(!empty($user_device_tokens)){
					foreach($user_device_tokens as $deviceDetail){
						...
						$this->send_push_notification($deviceDetail->device_id,$deviceDetail->device_type,"Your DMS vehicle service is due on $serviceDate.","service_due",array(),"Service Due");
						
						$notficationArr                     	 =   new UserNotification();
						$notficationArr->user_id          		 =   $retailService["customer_id"];
						$notficationArr->nofication_string       =   "Your DMS vehicle service is due on $serviceDate.";
						$notficationArr->save();
					}
				}
