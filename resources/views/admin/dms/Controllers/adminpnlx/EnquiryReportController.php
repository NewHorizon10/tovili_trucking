...
*/
	...
	public function customerEnquiryReport(){
		...
		if ((Input::get())) {
			...
			if(!empty($searchData['enquiry_start_date']) && empty($searchData['enquiry_end_date'])){
				$date_from	=	date("Y-m-d",strtotime($searchData['enquiry_start_date']));
				$DB->whereDate('enquiries.enquiry_date',$date_from);
				$searchVariable	=	array_merge($searchVariable,array('enquiry_start_date' => $searchData['enquiry_start_date']));
			}elseif(empty($searchData['enquiry_start_date']) && !empty($searchData['enquiry_end_date'])){
				$date_from	=	date("Y-m-d",strtotime($searchData['enquiry_end_date']));
				$DB->whereDate('enquiries.enquiry_date',$date_from);
				$searchVariable	=	array_merge($searchVariable,array('enquiry_start_date' => $searchData['enquiry_start_date']));
			}elseif(!empty($searchData['enquiry_start_date']) && !empty($searchData['enquiry_end_date'])){
				$date_from	=	date("Y-m-d",strtotime($searchData['enquiry_start_date']));
				$date_to	=	date('Y-m-d',strtotime($searchData['enquiry_end_date']));
				$DB->whereBetween('enquiries.enquiry_date',[$date_from,$date_to]);
				$searchVariable	=	array_merge($searchVariable,array('enquiry_start_date' => $searchData['enquiry_start_date'],'enquiry_end_date' => $searchData['enquiry_end_date']));
			}
			foreach($searchData as $fieldName => $fieldValue){
				if($fieldValue != "" && ($fieldName != 'enquiry_start_date' && $fieldName != 'enquiry_end_date')){
					if($fieldName == 'dealer_id'){
						$DB->where("enquiries.dealer_id",$fieldValue);
					}else{
		...
		if(Auth::user()->user_role_id == ADMIN_STAFF_ROLE_ID){
			...
			if(!empty($assignedDealer)){
				$DB->whereIn('enquiries.dealer_id', $assignedDealer);
			}
		}
		$result 				= 	$DB
									->where('enquiries.is_deleted',0)
									->leftjoin('users', 'enquiries.dealer_id', '=', 'users.id')
									->select("enquiries.*","users.full_name",
		...
		// these variable is for searial number
		$record_per_page	=	Config::get("Reading.records_per_page");
		if(Input::get('page') > 0){
			$page_number		=	Input::get('page');
		}else{
			$page_number		=	1;
		}
		...
		return  View::make('admin.EnquiryReport.customerEnquiryReport', compact('result', 'dealersList','enquirylevel','customer_category','sourceOfInformation','customerOccupation','finencer_option','vehiclemodel','enquirymode','status_type','sales_consultant','searchVariable','sortBy','order','query_string','record_per_page','page_number'));
	}
	...
	public function exportCustomerEnquiryReportToExcel(){
		...
		$result 				= 	$DB
									...
									DB::raw("(SELECT name FROM cities WHERE id = enquiries.city) as city"),
									DB::raw("(SELECT name FROM states WHERE id = enquiries.state) as state"))
									->orderBy('enquiries.'.$sortBy, $order)
									->get()->toArray();	
									// echo "<pre>";print_r($result);die;						
									
												
		$thead = array();
		$thead[]		= array("S. No.","Dealer Name","Enquiry No.","Mode of Enquiry","Enquiry Date","Source of Information","Occupation","Test Ride","Booking Model","Booking Model Colour","Enquiry Status","Enquiry Level","Sales Consultant","Customer Name","Date of Birth","Owner Age","Gender","Mobile Number","Contact No.","Email Id","Address1","Address2","Pin","State","City","Delivery Date","Delivery Address1","Delivery Address2","Customer Category","Financier","Follow-up Count","Follow-up Date","Comparison Company","Comparison Model","Current Vehicle","Advance","Payment Mode");
		if(!empty($result)) {
			 $i = 1;
			foreach($result as $record) {
				...
				$thead[]						= 	array($serialNumber,$dealer_name,$enquiry_number,$enquiry_mode,$enquiry_date,$source_of_info_name,$customer_occupation_name,$test_drive_date,$vehicle_modal,$vehicle_color_name,$enquirystatus,$enquiry_level_name,$sales_consultant,$customer_name,$dob,$age,$gender,$mobile_number,$contact_number,$email,$address_1,$address_2,$zip,$state,$city,$delivery_date,$delivery_address_1,$delivery_address_2,$customer_category_name,$financer,$follow_up_count,$next_follow_up_date,$current_vehicle_brand_name,$current_vehicle_model,$current_vehicle,$advance,$finencer_option_name);
				
				$i++;
			}
		}								
		// echo '<pre>'; print_r($thead); die;					
		return  View::make('admin.EnquiryReport.customer_enquiry_export_excel', compact('thead'));
	...
	public function customerEnquiryFollowUpReport(){
			...
			foreach($searchData as $fieldName => $fieldValue){
				if($fieldValue != ""){
					if($fieldName == 'follow_up_date' || $fieldName == 'follow_end_date'  ){
						...
						$EN->where("enquiries.dealer_id",$fieldValue);
					}else{
						$EN->where("enquiries.$fieldName",'like','%'.$fieldValue.'%');
					}
				}
				$searchVariable	=	array_merge($searchVariable,array($fieldName => $fieldValue));
			}
        ...
        return View::make('admin.EnquiryReport.enquiryFollowUpReport', compact("result","sortBy","order","total_filtered_data", "query_string", "searchVariable","sales_consultant","outletList","dealersList"));
	}
	...
	public function exportEnquiryFolloeUpReportToExcel(){
        ...
        if ($searchData) {
			...
			foreach($searchData as $fieldName => $fieldValue){
				if($fieldValue != ""){
					if($fieldName == 'follow_up_date' || $fieldName == 'follow_end_date'  ){
						if(!empty($searchData["follow_up_date"]) && empty($searchData["follow_end_date"])){
							$EN->where("enquiries.next_follow_up_date","=",$searchData['follow_up_date']);
							
						}elseif(!empty($searchData["follow_up_date"]) && !empty($searchData["follow_end_date"])){
							$EN->whereBetween("enquiries.next_follow_up_date",array($searchData['follow_up_date'],$searchData['follow_end_date']));
		...
		$thead = array();
		$thead[]		= array("Dealer Name","Enquiry No.","Customer Name","Vehicle Type","Follow-up Date","Sales Consultant","Sales Consultant");
		if(!empty($result)) {
			foreach($result as $record) {
				...
				$thead[]						= 	array($dealer_name,$unique_number,$customer_name,$vehicle_modal,$contact_date,$follow_up_remark,$sales_consultant);
			}
		}								
		// echo '<pre>'; print_r($thead); die;					
		return  View::make('admin.EnquiryReport.enquiry_follow_up_export_excel', compact('thead'));
	...
	public function uploadCustomerEnquiryReport(){
		...
		return  View::make('admin.EnquiryReport.upload_customer_enquiry', compact('result' ,'searchVariable','sortBy','order','query_string','vehiclemodel'));
	}
	...
	public function exportuploadCustomerEnquiryReportToExcel(){
		...
		return  View::make('admin.EnquiryReport.upload_customer_enquiry_excel', compact('thead'));
	}
