...
*/
	public function login(){
		...
		if(Request::isMethod('post')){
				...
				if ($validator->fails()){
					...
					if (Auth::attempt($userdata)){
						...
						if(Input::get('remember_me') != '' && Input::get('remember_me') == 1){
							Cookie::queue("DMSerp_login_email", Input::get('email'),DMSERP_COOKIE_TIME);
							Cookie::queue("DMSerp_login_password", Input::get('password'),DMSERP_COOKIE_TIME);
							Cookie::queue("DMSerp_login_remember_me", Input::get('remember_me'),DMSERP_COOKIE_TIME);
						}else{
							Cookie::queue(Cookie::forget('DMSerp_login_email'));
							Cookie::queue(Cookie::forget('DMSerp_login_password'));
							Cookie::queue(Cookie::forget('DMSerp_login_remember_me'));
						}
			...
			$remember_email 	= 	Cookie::get('DMSerp_login_email');
			$remember_password 	= 	Cookie::get('DMSerp_login_password');
			$remember_me_box 	= 	Cookie::get('DMSerp_login_remember_me');
