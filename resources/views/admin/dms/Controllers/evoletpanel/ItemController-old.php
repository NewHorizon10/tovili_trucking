	...
	public function itemSave(){
		...
		if(!empty($thisData)){
			...
			if ($validator->fails()) {	
				...
				$id  = $item->id;
				if(!empty($id)){
					$unique_id		=	'#I'.date('m').''.date('y').'000'.$id;
	...
	*/
	public function itemView($itemId = 0){
		...
		$itemDetails	    =	DB::table('items')
								...
								->leftjoin('dropdown_managers as model_table', 'items.modal', '=', 'model_table.id')
								->leftjoin('dropdown_managers as item_type_table', 'items.item_type', '=', 'item_type_table.id')
								->leftjoin('dropdown_managers as material_type_table', 'items.material_type', '=', 'material_type_table.id')
								->leftjoin('dropdown_managers as purchase_table', 'items.purchase', '=', 'purchase_table.id')
								->leftjoin('dropdown_managers as distribution_table', 'items.distribution', '=', 'distribution_table.id')
								->leftjoin('dropdown_managers as requirement_type_table', 'items.requirement_type', '=', 'requirement_type_table.id')
								...
								->select('items.*', 'color_table.name as color_name','size_table.name as size_name','item_category.category_name as category_name','commodity_category.commodity_name as commodity_name','model_table.name as model_name','item_type_table.name as item_type_name','material_type_table.name as material_type_name','purchase_table.name as purchase_name','distribution_table.name as distribution_name','requirement_type_table.name as requirement_type_name'
								)
								->first();
		...
		return View::make("admin.items.view", compact("itemDetails","vendors"));
	...
	*/
		public function itemEdit($itemId = 0){
			...
			$item_category 		=   DB::table('item_category')->where('is_deleted',0)->pluck('category_name','id');	
		 	$commodity_category =   DB::table('commodity_category')->where('is_active',1)->pluck('commodity_name','id');
			$vendors 			=   DB::table('vendors')->where('is_active',1)->pluck('company_name','id');	
			return View::make("admin.items.edit", compact("itemDetails",'size',"color","vendors","item_category","commodity_category",'itemmodel','goodtype','materialtype','purchaseUnit','requirmentType'));
