...
*/
	public function login(){
		...
		if(Request::isMethod('post')){
			...
			if(!empty($formData)){
				...
				if ($validator->fails()){
					...
					if (Auth::attempt($userdata)){
						if(Auth::user()->user_role_id != ADMIN_STAFF_ROLE_ID && Auth::user()->user_role_id != SUPER_ADMIN_ROLE_ID){
							Auth::logout();
							Session::flash('error', 'Email or Password is incorrect.');
							return Redirect::to('/adminpnlx');
						}
						if(Input::get('remember_me') != '' && Input::get('remember_me') == 1){
							Cookie::queue("evoleterp_login_email", Input::get('email'),EVOLETERP_COOKIE_TIME);
							Cookie::queue("evoleterp_login_password", Input::get('password'),EVOLETERP_COOKIE_TIME);
							Cookie::queue("evoleterp_login_remember_me", Input::get('remember_me'),EVOLETERP_COOKIE_TIME);
						}else{
							Cookie::queue(Cookie::forget('evoleterp_login_email'));
							Cookie::queue(Cookie::forget('evoleterp_login_password'));
							Cookie::queue(Cookie::forget('evoleterp_login_remember_me'));
						}
						...
						if(Auth::user()->twofactor_security == 1){
							...
							return Redirect::to('adminpnlx/admin_two_step_verification');
						}
						return Redirect::intended('adminpnlx/dashboard')->with('message','You are now logged in!');
					}else{
			...
			$remember_email 	= 	Cookie::get('evoleterp_login_email');
			$remember_password 	= 	Cookie::get('evoleterp_login_password');
			$remember_me_box 	= 	Cookie::get('evoleterp_login_remember_me');
			return View::make('admin.login.index', compact('remember_email', 'remember_password', 'remember_me_box'));
		}
   }// end index()
			...
			if(!empty($userDetail)){
				if($userDetail->is_active == 1 ){
					if($userDetail->is_verified == 1 ){
						...
						return Redirect::to('/adminpnlx/forget_password')->with('error', trans('Your account has not been verified yet.'));
					}					
				}else{
