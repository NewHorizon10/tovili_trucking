...
class ToursController extends BaseController {
	...
	public function addTour(){
		...
		$cityList		=	DB::table('cities')
						   ...
						   ->pluck('name','id')
						   ->toArray();
		return view('admin.'.$this->model.'.add',compact('leadList','cityList'));
	}
	...
	public function editTour($id){
	 ...
	 if($res == false){
      ...
      return view('admin.'.$this->model.'.edit',compact('data','leadList','imageDetails','ToursLocationList'));
	} //end edit Tour
	...
	public function updateTour($id){
		...
		if(!empty($formData)){
			...
			if ($validator->fails()){
			    ...
			    return Redirect::route('admin.'.strtolower($this->model).'.index');
			}
		}else{
   ...
   /**
	...
	public function viewTour($id){
	  ...
	  if(empty($data)){
        ...
        $data      =  Tours::where('id',$id)
		 ...
		 $imageDetails 	=  TourAttachments::where('tour_id',$id)->select("attachment","id")->get();
		 $ToursLocationList 				= 	ToursLocation::where('tour_id',$id)->select("start_location","end_location","id")->get();
	   return view('admin.'.$this->model.'.view',compact('data','imageDetails','ToursLocationList'));
	}////end viewTour
	...
	public function exportTour(){
		...
		return view('admin.'.$this->model.'.export_tours',compact('result' ,'searchVariable','sortBy','order','query_string'));
	}
	...
	public function addMoreLocation(){
		...
		return view('admin.'.$this->model.'.add_more_location',compact("counter"));
	}
	...
	//echo '<pre>'; print_r($result); die;
		return view('admin.'.$this->model.'.listSharedLocations',compact('result' ,'searchVariable','sortBy','order','query_string','tourData'));
	}
