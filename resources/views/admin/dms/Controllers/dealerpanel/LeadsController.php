...
class LeadsController extends BaseController {
	...
	public function index(){
	    ...
	    $teamTicketsAssigned = User::where('user_role_id',STAFF_USER_ROLE_ID)
							->where("dealer_id",$dealer_id)
							->where("is_active",1)
							->where("is_deleted",0)
							->orderBy('full_name', 'ASC')
		...
		return view('dealerpanel.'.$this->model.'.index',compact('result' ,'searchVariable','sortBy','order','query_string','stateList','sales_consultant','teamTicketsAssigned'));
	}
	...
	public function addLeads(){
	 ...
	 $teamTicketsAssigned = User::where('user_role_id',STAFF_USER_ROLE_ID)
      ...
      return view('dealerpanel.'.$this->model.'.add',compact('opportunities','stateList','sales_consultant','teamTicketsAssigned'));
	}
    /*
	...
	public function editLeads($id){
	   ...
	   $sales_consultant	= User::where('user_role_id',STAFF_USER_ROLE_ID)
		...
		return view('dealerpanel.'.$this->model.'.edit',compact('opportunities','stateList','sales_consultant','teamTicketsAssigned','data','cityList','imageDetails'));
	}
     /*
     *
	...
	public function viewLeads($id){
        ...
        return view('dealerpanel.'.$this->model.'.view',compact('data','leadsComments','leadFollowups','leadLogs','leadAttachments','LeadCommentAttachments'));
    ...
    public function exportLead(){
	 ...
	 return view('dealerpanel.'.$this->model.'.export_lead',compact('result' ,'searchVariable','sortBy','order','query_string'));
    }
	
	public function importLeads(){
      return view('dealerpanel.'.$this->model.'.importLead');
	}
	public function saveImportLeads(){
		...
		if ($validator->fails()){
			...
			if ($_FILES["imported_file"]["size"] > 0) {
				...
				return Redirect::to('dealerpanel/leads-management');
