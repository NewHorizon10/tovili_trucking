...
*/
       ...
       public function index() {
			...
			$result 				= 	$DB
										...
										->where('retail_services.dealer_id',$dealer_id)
										->where('retail_services.is_close',0)
										->where('retail_services.service_date','!=','0000-00-00')
			...
			$vehiclemodel =  $this->getDropDownListBySlug('vehiclemodel');
       	    return  View::make('dealerpanel.'.$this->model.'.index',compact('result' ,'searchVariable','sortBy','order','query_string','stateList','vehiclemodel'));
       }
	...
	*/
		public function report_index() {
			...
			return  View::make('dealerpanel.'.$this->model.'.report_index',compact('result' ,'searchVariable','sortBy','order','query_string','stateList','vehiclemodel','totalHighServices'));
	   }
	   ...
	   public function export_report_index() {
			...
			$totalHighServices	=	DB::select(DB::raw("SELECT count(id) as recordcount  FROM `model_services` group by dropdown_manager_id  ORDER BY recordcount DESc limit 1"));
			$totalHighServices	=	(!empty($totalHighServices[0]->recordcount)) ? $totalHighServices[0]->recordcount : 6;
			return  View::make('dealerpanel.'.$this->model.'.export_report_index',compact('result' ,'searchVariable','sortBy','order','query_string','stateList','vehiclemodel','totalHighServices'));
	   }
	  ...
	  */
       ...
       public function exportServiceReminderReportToExcel(){
		...
		return  View::make('dealerpanel.ServiceReminder.serviceReminderReport', compact('result' ,'searchVariable','sortBy','order','query_string','stateList','vehiclemodel'));
     ...
     public function followUpService($id){
		...
		if(empty($retailServices)) {
			...
			return view('dealerpanel.ServiceReminder.followUp',compact("retailServices","followUpDetails"));
		}
    ...
    public function addFollowUpService(){
        ...
        /*
         * Function for Crone Message curl
         *
         *
		 */
		public function cronMessage($phone_number,$message){
	         $ch = curl_init();
