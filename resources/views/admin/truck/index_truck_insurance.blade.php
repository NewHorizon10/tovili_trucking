...
<div class="content  d-flex flex-column flex-column-fluid" id="kt_content">
		...
		<div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
			<div class="d-flex align-items-center flex-wrap mr-1">
				<div class="d-flex align-items-baseline flex-wrap mr-5">
					...
					{{trans("messages.Truck Insurance List")}}
					</h5>
	...
	<div class="d-flex flex-column-fluid">
		<div class=" container ">
			<form action="{{ route($model.'.insurance.index')}}" method="get" clas="kt-form kt-form--fit mb-0" autocomplete="off">
				<div class="row">
					<div class="col-12">
						<div class="card card-custom card-stretch card-shadowless">
							<div class="card-header">
								...
								<div class="card-toolbar">
									...
									<a href='{{route("$model.insurance.export")}}' class="btn btn-primary">
							...
							<div class="card-body">
										...
										<div>
											<div class="row mb-6">
												...
												<div class="col-lg-3 mb-lg-5 mb-6">
                                                    ...
                                                    <select name="select_truck_expiry_type" class="form-control select2init  @error('company_type') is-invalid @enderror" >
                                                        <option value="">{{trans("messages.select_truck_expiry_type")}}</option>
														<option value="expired_truck_insurance" {{$searchVariable && ($searchVariable['select_truck_expiry_type'] ?? '') == "expired_truck_insurance" ? 'selected' : '' }} >{{trans('messages.expired_truck_insurance')}}</option>
														<option value="near_to_expiry_truck_insurance" {{$searchVariable && ($searchVariable['select_truck_expiry_type'] ?? '') == "near_to_expiry_truck_insurance" ? 'selected' : '' }} >{{trans('messages.near_to_expiry_truck_insurance')}}</option>
                                                    </select>
												</div>
												<div class="col-lg-3 mb-lg-5 mb-6">
											...
											<div class="row mt-8">
												<div class="col-lg-12">
													...
													&nbsp;&nbsp;
													<a href='{{ route($model.'.insurance.index')}}' class="btn btn-secondary btn-secondary--icon">
														<span>
								...
								<a href="{{ route('truck.insurance.notification') }}" class="btn btn-primary mb-2 sendNotificationButton @if(!empty(Session::get('truck_insurance_ids'))) @else d-none @endif">{{trans("messages.Select")}} {{trans("messages.Notifications")}}</a>
										...
										<table class="table dataTable table-head-custom table-head-bg table-borderless table-vertical-center" id="taskTable">
											<thead>
												<tr class="text-uppercase">
													<th>
                                                 	 <input type="checkbox" name="checkbox" class="subscriptionCheckbox" id="subscriptionCheckboxMain"  data-check-type="all-ids" value="allCheck" @if(!empty(Session::get('truck_insurance_ids'))) {{ (count(Session::get('truck_insurance_ids')) == $allResultCount) ? 'checked' : '' }} @endif>
                                             	 	</th>
													<th class="{{(($sortBy == 'type_of_truck' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'type_of_truck' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
														<a href="{{route($model.'.insurance.index',array(	'sortBy' => 'type_of_truck',
                                                    ...
                                                    <th class="{{(($sortBy == 'truck_system_number' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'truck_system_number' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
														<a href="{{route($model.'.insurance.index',array(	'sortBy' => 'truck_system_number',
													...
													<th class="{{(($sortBy == 'company_name' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'company_name' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
														<a href="{{route($model.'.insurance.index',array(	'sortBy' => 'company_name',
													...
													<th
														...
														<a href="{{route($model.'.insurance.index',array(	'sortBy' => 'truck_insurance_expiration_date',
													...
													<th class="{{(($sortBy == 'is_active' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'is_active' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
														<a href="{{route($model.'.insurance.index',array(	'sortBy' => 'is_active',
											...
											<tbody>
												...
												<tr>
													<td>
	                                                  <input type="checkbox" name="send_notification" class="subscriptionCheckbox" data-check-type="ids" value="{{$result->id}}" @if(!empty(Session::get('truck_insurance_ids'))) {{ (in_array($result->id, Session::get('truck_insurance_ids'))) ? 'checked' : '' }} @endif>
	                                                </td>
                                                    <td>
                                                    ...
                                                    <td>
														@if($result->truck_insurance_expiration_date == null || ($result->truck_insurance_expiration_date != null && \Carbon\Carbon::parse($result->truck_insurance_expiration_date)->lte(now())  ))
...
<script>
    $(document).ready(function() {
        ...
        $('.subscriptionCheckbox').on('change', function() {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("truck.insurance.notification") }}',
                data: {
					checkType: checkType,
					id: id,
					idSelected: idSelected,
