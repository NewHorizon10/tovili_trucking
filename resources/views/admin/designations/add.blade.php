		...
		<div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
			<div class="d-flex align-items-center flex-wrap mr-1">
				<div class="d-flex align-items-baseline flex-wrap mr-5">
					...
					{{trans('messages.add_new_role')}} </h5>
					<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
						<li class="breadcrumb-item">
							<a href="{{ route('dashboard')}}" class="text-muted">{{ trans('messages.Dashboard') }}</a>
						</li>
						<li class="breadcrumb-item">
							<a href="{{route($model.'.index')}}" class="text-muted">{{trans('messages.roles')}}</a>
						</li>
					</ul>
				</div>
			</div>
										...
										<div class="card-header d-flex align-items-center" role="tab">
											<div class="ml-5">
												<label class="checkbox">
													<input type="checkbox" name="data[{{$counter}}][value]" value=1 class="parent parent_{{$aclModule->id}}" id="{{$aclModule->id}}" {{ ($aclModule->active == 1) ? 'true' : 'false' }}>
												...
												<div class="">
													...
													foreach ($aclModule['sub_module'] as $subModule) {
															...
															if (!$subModule['module']->isEmpty()) {
																foreach ($subModule['module'] as $module) {
																	...
																	<div class="col-auto mb-5">
																		<label class="checkbox">
																			<input type="checkbox" id="{{$aclModule->id}}"  name="data[{{$counter}}][module][{{$module_counter}}][value]" value=1 class="childernAll childern_{{$aclModule->id}} children child_{{$aclModule->id}} @if($aclModule['is_third_level']==1) for_third_level_all_check @endif all_check_from_childern_{{$newTitle}}" data-parent-id="{{$aclModule->id}}" data-title="{{$newTitle}}" {{ ($module->active == 1) ? 'true' : 'false' }}>
																...
																<div class="row">
																	...
																	if (!empty($third_level_module['module'])) {
																		...
																		foreach ($third_level_module['module'] as $module) {
																			...
																			<div class="col-auto mb-5">
																				<label class="checkbox">
																					<input type="checkbox" id="{{$aclModule->id}}" class="children child{{$aclModule->id}} for_third_level_check target_all_check_checkobx_{{$newTitle}}" data-title="{{$newTitle}}" data-function-name="{{$function_name}}"  {{ ($module->active == 1) ?  'checked' : '' }}>
														...
														<div class="font-size-lg font-weight-bold mb-3 @if($aclModule['is_third_level']==1) d-none @endif">
															{{!empty($subModule->title)?strtoupper($subModule->title):''}}
														</div>
														...
														<div class="row @if($aclModule['is_third_level']==1) d-none @endif">
															...
															<div class="col-auto mb-5">
																<label class="checkbox">
																	<input type="checkbox" id="{{$aclModule->id}}"  name="data[{{$counter}}][module][{{$module_counter}}][value]" value=1 class="children child{{$aclModule->id}} target_checkobx_{{$newTitle}}_{{$function_name}} target_all_check_checkobx_{{$newTitle}}" {{ ($module->active == 1) ? 'true' : 'false' }}>
																	<input type="hidden" name="data[{{$counter}}][module][{{$module_counter}}][id]" value="{{$module->id}}">
																	<input type="hidden" name="data[{{$counter}}][module][{{$module_counter}}][designation_module_id]" value="{{$subModule->id}}">
																	<span class="mr-2"></span>
																	{{$module->name}}
