            ...
            <div class="d-flex align-items-center flex-wrap mr-1">
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    <h5 class="text-dark font-weight-bold my-1 mr-5">
                        {{trans('messages.Shipment_Request')}}
                    </h5>
                        ...
                        <div class="card card-custom card-stretch card-shadowless">
                            <div class="card-header">
                                ...
                                <div class="card-toolbar">
                                    ...
                                    <a href="{{route($model.'.export')}}" class="btn btn-primary mr-2">
                                            ...
                                            <div class="row mb-6">
                                                ...
                                                <div class="col-lg-3  mb-lg-5 mb-6">
                                                    ...
                                                    <select name = "status" class="form-control select2init">
                                                        <option value = "" {{ isset($searchVariable['status']) && $searchVariable['status'] == '' ? 'selected': '' }}>{{trans("messages.admin_All")}}</option>
                                                        <option value = "new" {{ isset($searchVariable['status']) && $searchVariable['status'] == 'new' ? 'selected': '' }}>{{trans("messages.new")}}</option>
                                                        <option value = "offers" {{ isset($searchVariable['status']) && $searchVariable['status'] == 'offers' ? 'selected': '' }}>{{trans("messages.offers")}}</option>
                                                        <option value = "offer_chosen" {{ isset($searchVariable['status']) && $searchVariable['status'] == 'offer_chosen' ? 'selected': '' }}>{{trans("messages.offer_chosen")}}</option>
                                                        <option value = "rejected" {{ isset($searchVariable['status']) && $searchVariable['status'] == 'rejected' ? 'selected': '' }}>{{trans("messages.offer_rejected")}}</option>
                                                    </select>
                                                </div>
                                                <div class="col-lg-3 mb-lg-5 mb-6">
                                                    <label>{{trans("messages.name")}}</label>
                                                    <input type="text" class="form-control" name="name" placeholder="{{trans("messages.name")}}" value="{{$searchVariable['user_name'] ?? '' }}">
                                                </div>
                                        ...
                                        <table class="table dataTable table-head-custom table-head-bg table-borderless table-vertical-center" id="taskTable">
                                            <thead>
                                                ...
                                                <tr class="text-uppercase">
                                                    ...
                                                    <th class="{{(($sortBy == 'user_name' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'user_name' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route($model.'.index',array(	'sortBy' => 'user_name',
                                                    ...
                                                    <th class="{{(($sortBy == 'request_types' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'request_types' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route($model.'.index',array(	'sortBy' => 'request_types',
													'order' => ($sortBy == 'request_types' && $order == 'desc') ? 'asc' : 'desc',	
													$query_string))}}">{{trans("messages.request_type")}}</a>
                                                    </th>
                                                    <th class="">
                                                        {{ trans('messages.admin_common_Price') }}
                                                    </th>

                                                    <th class="{{(($sortBy == 'request_date' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'request_date' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route($model.'.index',array(	'sortBy' => 'request_date',
													'order' => ($sortBy == 'request_date' && $order == 'desc') ? 'asc' : 'desc',	
													$query_string))}}">{{trans("messages.request_date")}}</a>
                                                    </th>
                                                  
                                                    <th class="{{(($sortBy == 'pickup_address' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'pickup_address' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route($model.'.index',array(	'sortBy' => 'pickup_address',
													'order' => ($sortBy == 'pickup_address' && $order == 'desc') ? 'asc' : 'desc',	
													$query_string))}}">{{trans("messages.pickup_address")}}</a>
                                                    </th>

                                                    <th class="{{(($sortBy == 'shipment_offers_count' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'shipment_offers_count' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route($model.'.index',array( 'sortBy' => 'shipment_offers_count',
                                                    'order' => ($sortBy == 'shipment_offers_count' && $order == 'desc') ? 'asc' : 'desc',  
                                                    $query_string))}}">{{trans("messages.offers")}}</a>
                                                    </th>
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="text-dark-75 mb-1 font-size-lg">
                                                            {{ empty($result->user_name) ? '--' : $result->user_name }}
                                                    ...
                                                    <td>
                                                        ...
                                                        @if($result->shipmentOffersCount ?->count() == 0)

                                                        <a href="{{ route($model.'.show',$result->request_number) . '?tabs=offers' }}" class="text-dark-75">
                                                        {{ isset($result->approvedShipmentPrice->price) ? number_format($result->approvedShipmentPrice->price, 2) : (isset($result->rejectedShipmentOffersCount) && $result->rejectedShipmentOffersCount->count() > 0 ? $result->rejectedShipmentOffersCount->count() . ' offer'  : '--') }}
                                                        </a>
                                                         @elseif($result->shipmentOffersCount ?->count() > 1)
                                                           <a href="{{ route($model.'.show',$result->request_number) . '?tabs=offers' }}" class="text-dark-75">  {{ $result->shipmentOffersCount ?->count() }} offers </a>
                                                           @elseif($result->shipmentOffersCount ?->count() == 1)
                                                           <a href="{{ route($model.'.show',$result->request_number) . '?tabs=offers' }}" class="text-dark-75">  {{ $result->shipmentOffersCount ?->count() }} offer </a>
                                                           @else 
                                                              --
                                                        @endif
                                                    ...
                                                    <td>
                                                        <div class="text-dark-75 mb-1 font-size-lg">
                                                            {{  date(config("Reading.date_format"),strtotime($result->request_date)) }}
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td>
                                                        <div class="text-dark-75 mb-1 font-size-lg">
                                                            <a href="javascript:void(0);" class="btn btn-icon btn-light btn-hover-primary btn-sm offercounthoverparent">
                                                                {{ $result->shipment_offers_count }}
                                                                
                                                                <ul class="offercountchild">
                                                                    <li>{{ trans('messages.waiting'). ': ' .$result->waiting_offer ?? 0 }}</li>
                                                                    <li>{{ trans('messages.selected'). ': ' .$result->selected_offer ?? 0 }}</li>
                                                                    <li>{{ trans('messages.rejected'). ': ' .$result->rejected_offer ?? 0 }}</li>
                                                                </ul>
                                                            </a>
                                                    ...
                                                    <td>
                                                        @if($result->shipment_status && $result->status != "cancelled")
                                                            ...
                                                            @endif
                                                        @else
                                                            @if($result->status == 'offer_chosen')
                                                                 {{ trans('messages.admin_waiting_for_confirmation') }}
                                                            @else
                                                    ...
                                                    <td class="text-right pr-2">
                                                        <a href="{{route($model.'.show',$result->request_number)}}" class="btn btn-icon btn-light btn-hover-primary btn-sm" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" title="" data-original-title="{{trans('messages.admin_common_View')}}">
