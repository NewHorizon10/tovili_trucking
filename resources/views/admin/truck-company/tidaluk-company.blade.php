...
?>
    ...
    <div class="d-flex flex-column-fluid">
        <div class=" container ">
            {{ Form::open(['method' => 'get','role' => 'form','route' => "$model.tidaluk-company",'class' => 'kt-form kt-form--fit
                    ...
                    <div class="card card-custom card-stretch card-shadowless">
                        <div class="card-header">
                            ...
                            <div class="card-toolbar">
                                ...
                                <a href='{{route("$model.create", ['from_page=tidaluk_company'])}}' class="btn btn-primary mr-2">
                                    {{trans("messages.add_new_truck_company")}}
                                </a>

                                <a href='{{route("$model.tidaluk-company-export")}}' class="btn btn-primary">
                        ...
                        <div class="card-body">
                                ...
                                <div id="collapseOne6"
                                    ...
                                    <div>
                                        <div class="row mb-6">
                                            <div class="col-lg-3 mb-lg-5 mb-6">
                                                ...
                                                <select name="company_tidaluk" class="form-control select2init" >
                                                    <option value="">{{trans("messages.admin_All")}}</option>
                                                    <option value="not_selected" {{($searchVariable['company_tidaluk'] ?? '') == "not_selected" ? "selected" : '' }}>{{trans('messages.not_selected')}}</option>
                                                    @foreach($tidalukCompanyList as $tidalukCompany)
                                                        <option value="{{$tidalukCompany->id}}" {{($searchVariable['company_tidaluk'] ?? '') == $tidalukCompany->id ? "selected" : '' }}>{{$tidalukCompany->lookupDiscription->code}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                    ...
                                    <div class="row mt-8">
                                        <div class="col-lg-12">
                                            ...
                                            &nbsp;&nbsp;
                                            <a href='{{ route("$model.tidaluk-company")}}'
                         ...
                         <a href="{{ route('truck.company.tidaluk.company.notification') }}" class="btn btn-primary mb-2 sendNotificationButton @if(!empty(Session::get('tidaluk_company_ids'))) @else d-none @endif">{{trans("messages.Select")}} {{trans("messages.Notifications")}}</a>
                                ...
                                <table
                                    ...
                                    <thead>
                                        <tr class="text-uppercase">
                                            <th>
                                             <input type="checkbox" name="checkbox" class="subscriptionCheckbox" id="subscriptionCheckboxMain"  data-check-type="all-ids" value="allCheck" @if(!empty(Session::get('tidaluk_company_ids'))) {{ (count(Session::get('tidaluk_company_ids')) == $allResultCount) ? 'checked' : '' }} @endif>
                                            </th>
                                            <th
                                                class="{{(($sortBy == 'id' && $order == 'desc') ? '' : (($sortBy == 'id' && $order == 'asc') ? '' : ''))}}">
                                                <a href="{{route($model.'.tidaluk-company',array(	'sortBy' => 'id',
                                            ...
                                            <th
                                                ...
                                                <a href="{{route($model.'.tidaluk-company',array(	'sortBy' => 'company_name',
                                             ...
                                             <th
                                                ...
                                                <a href="{{route($model.'.tidaluk-company',array( 'sortBy' => 'phone_number',
                                            ...
                                            <a href="{{route($model.'.tidaluk-company',array( 'sortBy' => 'company_tidaluk',
                                                'order' => ($sortBy == 'company_tidaluk' && $order == 'desc') ? 'asc' : 'desc', 
                                                $query_string))}}">{{trans('messages.admin_Tidaluk_Company')}}</a>
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>
                                              <input type="checkbox" name="send_notification" class="subscriptionCheckbox" data-check-type="ids" value="{{$result->id}}" @if(!empty(Session::get('tidaluk_company_ids'))) {{ (in_array($result->id, Session::get('tidaluk_company_ids'))) ? 'checked' : '' }} @endif>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                ...
                                                <div class="text-dark-75 mb-1 font-size-lg" {!! $result->userCompanyInformation ?->getCompanyTidalukCompanyDescription == null ? 'style="color: #714b05 !important;"' : '' !!} >
                                                    {{ $result->userCompanyInformation ?->getCompanyTidalukCompanyDescription ?->code ?? trans('messages.not_selected')}}
                                                </div>
                                            </td>
...
<script>
    $(document).ready(function() {
        ...
        var SessionArr = '@if(!empty(Session::get("tidaluk_company_ids"))){{ Count(Session::get("tidaluk_company_ids")) }} @endif';
        ...
        $('.subscriptionCheckbox').on('change', function() {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("truck.company.tidaluk.company.notification") }}',
                data: {
                    checkType: checkType,
                    id: id,
                    idSelected: idSelected,
