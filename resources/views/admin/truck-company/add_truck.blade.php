...
<div class="content  d-flex flex-column flex-column-fluid" id="kt_content">
            ...
            <div class="d-flex align-items-center flex-wrap mr-1">
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    ...
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        ...
                        <li class="breadcrumb-item">
                            <a href="{{ route($model.'.index_truck',[$entruckid])}}" class="text-muted"> {{trans("messages.Truck Details")}}</a>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    <div class="d-flex flex-column-fluid">
        <div class="container">
            <form action="{{route($model.'.truck_save',[$entruckid])}}" method="post" class="mws-form" autocomplete="off" enctype="multipart/form-data">
                @php
                $i = 1;
                @endphp
                @csrf
                @if($TCuserid)
                    <input type="hidden" name="tc_id" value="{{request('tc_id')}}">
                    ...
                    <div class="card-body">
                        <div class="row">
                            @if($entruckid == null)
                                <div class="col-xl-6">
                                    <div class="form-group">
                                        <label for="type_of_truck">{{trans("messages.admin_Truck_Company")}}</label><span class="text-danger"> * </span>
                                        <select id="selectTruckCompany" name="truck_company" class="form-control select2init  @error('truck_company') is-invalid @enderror 1-form-fields" data-is-required="1" data-form-step="{{$i}}" >
                                            <option value="">{{trans("messages.select_truck_company")}}</option>
                                            @foreach($truckCompanies as $row)
                                                    <option value="{{$row->company_id}}">{{$row->company_name}}</option>
                                            @endforeach
                                        </select>
                                        <div class=" invalid-feedback error-truck_type">
                            ...
                            <div class="col-xl-6">
                                <div class="form-group">
                                    ...
                                    <select name="truck_type" class="form-control select2init truck_types @error('truck_type') is-invalid @enderror 1-form-fields" data-is-required="1" data-form-step="{{$i}}" >
                                        <option value="">{{trans("messages.admin_common_select_truck_type")}}</option>
                                        @foreach($truckType as $row)
                                            <option value="{{$row->id}}" {{old('truck_type') == $row->id ? 'selected' : '' }} >{{$row->name}}</option>
                                        @endforeach
                                    </select>
                                    <div class=" invalid-feedback error-truck_type">
                                        @if ($errors->has('truck_type'))
                            ...
                            <div class="col-xl-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="truck_system_number" class="form-control form-control-solid form-control-lg  @error('truck_system_number') is-invalid @enderror 1-form-fields" data-is-required="1" value="{{old('truck_system_number')}}">
                            ...
                            @foreach($truckType as $row)
                                <div class="col-xl-12 row truckTypeQuestionnaireClass{{$i}}" id="truckTypeQuestionnaire-{{$i}}-{{$row->id}}" style="display: none;">
                            ...
                            <div class="col-xl-6">
                                <div class="form-group">
                                    <label for="truck_insurance_picture">{{trans("messages.Upload_an_Insurance_certificate")}}</label>
                                    <input type="file" accept="image/png, image/gif, image/jpeg" name="truck_insurance_picture" class="form-control form-control-solid form-control-lg  @error('truck_insurance_picture') is-invalid @enderror 1-form-fields" data-is-required="0" value="{{old('truck_insurance_picture')}}">
                            ...
                            <div class="col-xl-6">
                                ...
                                <div class="input-group date" id="truck_insurance_expiration_date_id" data-target-input="nearest">
                                    {{ Form::text('truck_insurance_expiration_date',(
                                        ...
                                        (old('truck_insurance_expiration_date') ?? '')
                            ...
                            <div class="col-xl-6">
                                <div class="form-group">
                                    <label for="truck_licence_number">{{trans("messages.Upload_an_truck_license")}}</label>
                                    <input type="file" accept="image/png, image/gif, image/jpeg" name="truck_licence_number" class="form-control form-control-solid form-control-lg  @error('truck_licence_number') is-invalid @enderror 1-form-fields" data-is-required="0" value="{{old('truck_licence_number')}}">
                            ...
                            <div class="col-xl-6">
                                ...
                                <div class="input-group date" id="truck_licence_expiration_date" data-target-input="nearest">
                                    {{ Form::text('truck_licence_expiration_date',(
                                        ...
                                        (old('truck_licence_expiration_date') ?? '')
                            ...
                            <div class="col-xl-6">
                                <div class="form-group">
                                    ...
                                    <select id="selectDriver" name="driver_id" class="form-control select2init  @error('driver_id') is-invalid @enderror 1-form-fields" data-is-required="0" >
                                        <option value="">{{trans("messages.selecte_driver")}}</option>
                                        @foreach($free_driver as $row)
                                            <option value="{{$row->id}}" {{old('driver_id') == $row->id ? 'selected' : '' }} >{{$row->name}}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="d-flex justify-content-between border-top mt-5 pt-10">
                            <div class="row">
                                ...
                                <div class="col-6">
                                    <a type="button" href="{{($entruckid ? route('truck-company.edit', [$entruckid, 'tabs=truck_detail']) : route('truck-company.all-truck-list'))}}" class="btn btn-danger font-weight-bold text-uppercase px-9 py-4">
...
<script>
    ...
    $(document).ready(function(){
     $("#selectTruckCompany").on("change", function(){

        $("#selectDriver").html('<option value="">{{trans("messages.selecte_driver")}}</option>');
        var option = '';
        var company_id = $(this).val();
        
        $.ajax({
            url:"{{ route('fetch-truck-drivers') }}",
            type: "GET",
            data:{
                company_id:company_id
            },
            success:function(response){
                $.each(response.driverLists, function(index, value){
                  option += `<option value="${value.id}">${value.name}</option>`;
                   $("#selectDriver").html(option);
                });
            }
        });
