                                        ...
                                        <div>
                                            <div class="row mb-6">
                                                ...
                                                <div class="col-lg-3 mb-lg-5 mb-6">
                                                    ...
                                                    <input type="text" name="name" class="form-control" value="{{ isset($searchVariable['name']) ? $searchVariable['name'] : '' }}" placeholder="{{trans("messages.name")}}">
                                                </div>                                               
                                                <div class="col-lg-3 mb-lg-5 mb-6">
                                                    <label>{{trans("messages.Email Address")}}</label>
                                                    <input type="text" name="email" class="form-control" value="{{ isset($searchVariable['email']) ? $searchVariable['email'] : '' }}" placeholder="{{trans("messages.Email Address")}}">
                                                </div>
                                                 <div class="col-lg-3 mb-lg-5 mb-6">
                                                    <label>{{trans("messages.admin_phone_number")}}</label>
                                                    <input type="text" name="phone_number" class="form-control" value="{{ isset($searchVariable['phone_number']) ? $searchVariable['phone_number'] : '' }}" placeholder="{{trans("messages.admin_phone_number")}}">
                                                </div>
                                            </div>
                                        ...
                                        <table class="table dataTable table-head-custom table-head-bg table-borderless table-vertical-center" id="taskTable">
                                            <thead>
                                                <tr class="text-uppercase">
                                                    ...
                                                    'order' => ($sortBy == 'name' && $order == 'desc') ? 'asc' : 'desc', $query_string))}}">
                                                    {{trans("messages.name")}}</a>
                                                    </th>
                                                    ...
                                                    {{trans("messages.Email Address")}} </a>
                                                    </th>
                                                    ...
                                                    {{trans("messages.admin_phone_number")}} </a>
                                                    </th>

                                                    <th class="{{(($sortBy == 'comments' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'comments' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
                                                        <a href="{{route($modelName.'.index',array( 'sortBy' => 'comments',
                                                    'order' => ($sortBy == 'comments' && $order == 'desc') ? 'asc' : 'desc',  $query_string))}}">
                                            ...
                                            <tbody>
                                                ...
                                                <tr class="tr-row-class">
                                                    ...
                                                    <td>
                                                        <div class="text-dark-75 mb-1 font-size-lg fix-text-lenth" title="{{ $result->comments }}">
                                                            {{ Illuminate\Support\Str::limit($result->comments, 20) . (str_word_count($result->comments) > 20 ? '...' : '') }}
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td class="text-right pr-2">
                                                        ...
                                                        <a href='{{ route("$modelName.show", base64_encode($result->id)) }}' class="btn btn-icon btn-light btn-hover-primary btn-sm" data-toggle="tooltip" data-placement="top" data-container="body" data-boundary="window" title="" data-original-title="{{trans('messages.admin_common_View')}}">
