...
<head>
    ...
    <link href="{{ asset('./public/css/style.bundle.css')}}?4" rel="stylesheet" type="text/css" />
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/socket.io/4.5.0/socket.io.js"></script>
    ...
    <script>
        ...
        socket.on( 'connect', function() {
            let userId = "user_"+{{ Auth::guard('admin')->user()->id }};
        ...
        socket.on( 'sendEmitMessageResponce', function(data) {
            ...
            toastr.success(data.message,data.sender_name);
        ...
        const allowimagetypes = ['image/png','image/jpg','image/jpeg'];
    </script>
    ...
    <div id="kt_header_mobile" class="header-mobile align-items-center  header-mobile-fixed ">
        <a href="{{route('adminpnlx')}}">
            <img width="50px" alt="Logo" src="{{asset('./public/img/logo-white.png')}}" />
        </a>
    ...
    <div class="d-flex flex-column flex-root">
        <div class="d-flex flex-row flex-column-fluid page">
            ...
            $languages = DB::table('languages')->where('is_active',1)->orderBy('id','desc')->get();
                    ...
                    <div class=" container-fluid  d-flex align-items-stretch justify-content-between">
                        <div class="topbar ml-auto">
                            <div class="dropdown ml-3">
                                <form id="contactsForm" method="get" action="">
                                    <input type="hidden" name="_token" value="DCPr0MSQQZYK7jPH5MSLCIlZhik5nudQYajzAnxZ">
                                    <div class="form-group">
                                        <div class="justify-content-between mt-2 mx-5">
                                            <select class="form-control form-control-solid bg-white h-auto py-5 px-6 rounded-lg border-0 website_role_id" name="website_role_id">	
                                                @foreach($languages as $key => $value)	
                                                    <option value="{{$value->lang_code}}" {{Session::get('admin_applocale') == $value->lang_code ? "selected": "" }}>{{$value->title}}</option>	
                                                @endforeach	
                                             </select> 
                                        </div>
                                    </div>
                                ...
                                <div class="topbar-item" data-toggle="dropdown" data-offset="10px,0px">
                                    <a href="#" class="d-flex align-items-center">
                                        ...
                                        <div class="d-flex flex-column text-left">
                                            <span class="text-muted font-weight-bold">{{trans('messages.admin_welcome')}}</span>
                                    ...
                                    <div class="row row-paddingless">
                                        <div class="col-6">
                                            <a href="{{URL::to('adminpnlx/myaccount')}}" class="d-block py-6 px-5 text-center bg-hover-light border-right border-bottom">
                                                ...
                                                </span> <span class="d-block text-dark-75 font-weight-bold font-size-h10 mt-2 mb-1">{{trans('messages.admin_edit_profile')}}</span>
                                            </a>
                                        </div>
                                        <div class="col-6">
                                            <a href="{{{route('changedPassword')}}}" class="d-block py-6 px-5 text-center bg-hover-light border-right border-bottom">
                                                ...
                                                </span> <span class="d-block text-dark-75 font-weight-bold font-size-h10 mt-2 mb-1">{{trans('messages.Change Password')}}</span>
                                            </a>
                                        </div>
                                        <div class="col-12">
                                            <a href="{{URL::to('adminpnlx/logout')}}" class="d-block py-6 px-5 text-center bg-hover-light border-bottom">
                                                ...
                                                </span> <span class="d-block text-dark-75 font-weight-bold font-size-h10 mt-2 mb-1">{{trans('messages.Logout')}}</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
    ...
    <div id="videoModal" class="modal fade" aria-modal="true" role="dialog">
        <div class="modal-dialog">
    
        <!-- Modal content-->
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="themeBtn-close" data-dismiss="modal"><svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M16.6667 16.302L2.00004 2M16.6667 2L2 16.302" stroke="white" stroke-width="2.5" stroke-linecap="round"></path>
                    </svg>
                </button>
                <h4 class="modal-title">{{ trans('messages.admin_video') }}</h4>
            </div>
            <div class="modal-body">
                <center>
                <video width="400" controls="" id="video" src="">
                    <source src="" type="video/mp4">
                    <source src="mov_bbb.ogg" type="video/ogg">
                    {{ trans('messages.your_browser_does_not_support_HTML_video') }}
                </video>
                </center>
            </div>
            <div class="text-center d-block" style="margin-bottom: 12px;">
                <button type="button" class="themeModalBtn" data-dismiss="modal">Close</button>
            </div>
        </div>
    ...
    <script>
        ...
        $.getCountXHat = function() {
            $.ajax({
                type: 'get',
                url: '{{ route('get_chat_count') }}',
                data: {},
                headers: {},
			    
                success: function(response) {
                    $(".chatClass").html(response.data.total)
                },
        ...
        $.getCountXHat();
            ...
            $(function () {
                var tableContainer = $(".table-responsive-new");
                var table = $(".table-responsive-new table");
                var fakeContainer = $(".dataTables_wrapper-fake-top-scroll");
                var fakeDiv = $(".dataTables_wrapper-fake-top-scroll div");
            ...
            $('.website_role_id').on('change', function() {
                var website_role_id = $('.website_role_id :selected').val();
                $('#contactsForm').attr('action', "{{URL::to('change-language-settings/')}}/" + website_role_id).submit();
            });
            ...
            $(".status_any_item").click(function(e) {
                ...
                Swal.fire({
                    title: "{{trans('messages.admin_common_Are_you_sure')}}",
                    text: "{{trans('messages.admin_Want_to_change_this_status')}}",
                    icon: "warning",
                    showCancelButton: true,
                    confirmButtonText: "{{trans('messages.admin_Yes_change_it')}}",
                    cancelButtonText: "{{trans('messages.admin_No_cancel')}}",
                    reverseButtons: true
                }).then(function(result) {
                    if (result.value) {
