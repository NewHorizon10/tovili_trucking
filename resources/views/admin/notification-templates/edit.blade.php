...
<div class="content  d-flex flex-column flex-column-fluid" id="kt_content">
			...
			<div class="d-flex align-items-center flex-wrap mr-1">
				<div class="d-flex align-items-baseline flex-wrap mr-5">
					<h5 class="text-dark font-weight-bold my-1 mr-5">
						{{trans("messages.admin_common_edit_Notification_Template")}}
					</h5>
					<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
						...
						<li class="breadcrumb-item">
							<a href="{{ route($model.'.index')}}" class="text-muted">{{trans("messages.admin_common_Notification_Templates")}}</a>
						</li>
					</ul>
				</div>
			</div>
	...
	<div class="d-flex flex-column-fluid">
		<div class=" container ">
			<form action="{{route($model.'.update',base64_encode($notificationTemplate->id))}}" method="post" class="mws-form">
				@csrf
				@method('PUT')
											...
											<div class="row">
												...
												<div class="col-xl-6" style="display:none;">
													<div class="form-group">
														...
														<select name="data[{{$language->id}}][action]" class="form-control select2init" id="action">
															<option value="">{{$notificationTemplate->action}}</option>
														</select>
													</div>
												</div>
												<div class="col-xl-6">
												...
												<div class="col-xl-12">
													<div class="form-group">
														...
														@if($i == 1)
														<lable for="{{$language->id}}.'.body'">{{trans("messages.admin_common_Notification_Body")}}</lable><span class="text-danger"> * </span>
														<textarea id="body_{{$language->id}}" name="data[{{$language->id}}][body]" class="form-control form-control-solid form-control-lg  @error('body') is-invalid @enderror" value="{{old('body')}}">
															{{$multiLanguage[$language->id]['body'] ?? ''}} 
														</textarea>
														@if ($errors->has('body'))
														<div class="invalid-feedback">
															{{ $errors->first('body') }}
