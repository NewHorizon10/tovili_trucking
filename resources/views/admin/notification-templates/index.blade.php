		...
		<div class=" container-fluid  d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
			<div class="d-flex align-items-center flex-wrap mr-1">
				<div class="d-flex align-items-baseline flex-wrap mr-5">
					...
					{{trans("messages.admin_common_Notification_Templates")}}
					</h5>
										...
										<div>
											<div class="row mb-6">
												...
												<div class="col-lg-3 mb-lg-5 mb-6">
													...
													<input type="text" class="form-control" name="name" placeholder="{{trans("messages.name")}}" value="{{$searchVariable['subject'] ?? ''}}">
												</div>
											</div>
											<div class="row mt-8">
												<div class="col-lg-12">
										...
										<table class="table dataTable table-head-custom table-head-bg table-borderless table-vertical-center" id="taskTable">
											<thead>
												...
												<th class="{{(($sortBy == 'name' && $order == 'desc') ? 'sorting_desc' : (($sortBy == 'name' && $order == 'asc') ? 'sorting_asc' : 'sorting'))}}">
													...
													<th>
														{{trans('messages.Syatem Notification Enable')}}
													</th>
													<th>
														{{trans('messages.Whatsapp Notification Enable')}}
													</th>
											...
											<tbody>
												...
												<td>
													...
													<td>
														<input type="checkbox" class="notification_checkbox" data-checked-type="system" data-system-id="{{$result->id}}" name="is_enable" value="1" {{($result->system_notification_enable == 1) ? 'checked' : ''}}>
													</td>
													<td>
														<input type="checkbox" class="notification_checkbox"  data-checked-type="whatsapp" data-whatsapp-id="{{$result->id}}" name="is_enable" value="1" {{($result->whatsapp_notification_enable == 1) ? 'checked' : ''}}>
													</td>
													<td>
														<div class="text-dark-75 mb-1 font-size-lg">
...
<script>
	$(document).ready(function(){
		$(".notification_checkbox").on("change", function(){
			...
			var checkedType = $(this).data("checked-type");
			if($(this).is(":checked")){
				if(checkedType == 'system'){
					checkedId    = $(this).data("system-id");
					checkedValue = $(this).val(); 
				} else if(checkedType == 'whatsapp'){
					checkedId    = $(this).data("whatsapp-id");
					checkedValue = $(this).val(); 
				} 
			}else {
				if(checkedType == 'system'){
			...
			$.ajax({
				url: "{{ route('notification-templates.admin-notification-template-notification-enable') }}",
				type: "get",
				data:{checkedType:checkedType, checkedId:checkedId, checkedValue:checkedValue},
