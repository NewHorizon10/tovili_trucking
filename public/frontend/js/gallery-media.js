...
function hasFileExtension(filename,filetype) {
    if(filetype == ".docx"){
...
function GalleryImages(files){
    ...
    // alert(hasFileExtension(fileName,));
    var ext      = files[0].type.split('/');
    if($("#drop-areaa").attr('data-max-file')){
        if($("#drop-areaa").attr('data-max-file') < $("#drop-areaa .gallery_item").length){
            toastr.error($("#drop-areaa").attr('data-max-file-messages'));
            return false;
        }
    }
    if (ext[0] == "image" || ext[0] == "video" || ( ext[0] == "application" && (hasFileExtension(fileName ,'.doc') || hasFileExtension(fileName ,'.docx') || hasFileExtension(fileName ,'.pdf') ))){
            ...
            success: function(data) {
                $.each(data,function(key,val) {
                    if (val.type){
                        if(val.type == 'video'){
                            var html = `
                                    <div class="gallery_item itemsappends video_block d-inline-flex">
                                        <input style="visibility: hidden;height:0px; position: absolute;" name="image[]" class="input_images_gallery_items" value="${val.image}">
                                        <input style="visibility: hidden;height:0px; position: absolute;" name="type[]" value="${val.type}">
                                        <div class="video-items">
                                            <a href="javascript:void(0);" data-bs-toggle="modal" data-src="${url+'/public/uploads/gallery-image/'+val.image}" class="video-btn play_icon">
                                                <video src="${url+'/public/uploads/gallery-image/'}${val.image}">
                                            ...
                                            <div class="gallery_action">
                                                <a class="dropdown-item DeleteImage DeleteImage-icon" data-image="${val.image}" href="javascript:void(0)">
                                                    <span>
                                ...
                                var html = `<div class="gallery_item itemsappends d-inline-flex">
                                                <input style="visibility: hidden;height:0px; position: absolute;" name="image[]" class="input_images_gallery_items" value="${val.image}">
                                                <input style="visibility: hidden;height:0px; position: absolute;" name="type[]" class="input_type_gallery_items" value="${val.type}">
                                                    <figure>
                                                        <a class="fancybox-buttons" data-fancybox-group="button" href="${url+'/public/uploads/gallery-image/'+val.image}">
                                                            <img src="${url+'/public/uploads/gallery-image/'+val.image}" alt="">
                                                        </a>
                                                    </figure>
                                                <div class="gallery_action">
                                                    <a class="DeleteImage DeleteImage-icon" data-image="${val.image}" href="javascript:void(0)">
                                                        <span>
                                                            <svg width="14" height="14" viewBox="0 0 10 10" fill="none"
                                                                xmlns="http://www.w3.org/2000/svg">
                                                                <g clip-path="url(#clip0_11_9049)">
                            ...
                            }else if(hasFileExtension(val.image ,'.pdf')){
                                var html = `<div class="gallery_item itemsappends video_block d-inline-flex">
                                            <input style="visibility: hidden;height:0px; position: absolute;" name="image[]" class="input_images_gallery_items" value="${val.image}">
                                            <input style="visibility: hidden;height:0px; position: absolute;" name="type[]" value="${val.type}">
                                            <div class="video-items">
                                                <a href="${url+'/public/uploads/gallery-image/'+val.image}" target=”_blank”>
                                                    <img src="${url+'/public/frontend/img/pdf-icon.png'}">
                                                        <source src="" type="video/mp4">
                                                    </video>
                                                </a>
                                                <div class="gallery_action">
                            ...
                            }else if(hasFileExtension(val.image ,'.docx') || hasFileExtension(val.image ,'.doc') ){
                            var html = `<div class="gallery_item itemsappends video_block d-inline-flex">
                                        <input style="visibility: hidden;height:0px; position: absolute;" name="image[]" class="input_images_gallery_items" value="${val.image}">
                                        <input style="visibility: hidden;height:0px; position: absolute;" name="type[]" value="${val.type}">
                                        <div class="video-items">
                                            <a href="${url+'/public/uploads/gallery-image/'+val.image}" target=”_blank”>
                                                <img src="${url+'/public/frontend/img/docx-icon.png'}">
                                                    <source src="" type="video/mp4">
                                                </video>
                                            </a>
                                            <div class="gallery_action">
