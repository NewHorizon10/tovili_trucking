   ...
   public function index(Request $request){
	...
	if ($request->all()){
		...
		if((!empty($searchData['date_from'])) && (!empty($searchData['date_to']))){
			...
			$DB->whereBetween('email_logs.created_at', [date('Y-m-d H:i:s',strtotime($dateS." 00:00:00")), date('Y-m-d H:i:s',strtotime($dateE." 23:59:59"))]); 											
		}elseif(!empty($searchData['date_from'])){
			$dateS = $searchData['date_from'];
			$DB->where('email_logs.created_at','>=' ,[date('Y-m-d H:i:s',strtotime($dateS." 00:00:00"))]); 
		}elseif(!empty($searchData['date_to'])){
			$dateE = $searchData['date_to'];
			$DB->where('email_logs.created_at','<=' ,[date('Y-m-d H:i:s',strtotime($dateE." 23:59:59"))]); 						
		}
