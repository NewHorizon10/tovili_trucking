...
class LookupsController extends Controller
{
    ...
    public function index(Request $request, $type = null)
    {
        ...
        $getId                =    Lookup::query()->where('lookup_type', $type)->pluck('id')->toArray();
        $DB                   =    LookupDiscription::with('LookupParentId')->whereIn('parent_id', $getId)->where('language_id', $langCode);
        ...
        if ($request->all()) {
            ...
            foreach ($searchData as $fieldName => $fieldValue) {
                if($fieldName == 'status' && $fieldValue != null){
                   $DB->whereHas('LookupParentId', function($query) use($fieldValue){
                     $query->where('is_active', $fieldValue);
                   });
                }
                if ($fieldName == "code") {
    ...
    public function add(Request $request, $type = null)
    {
        if ($request->isMethod('POST')) {
            ...
            if(!empty($type == 'skills')){
                Validator::extend('unique_skills', function($attribute,$value,$parameters){
                    $getinfo  =   DB::table('lookups')->where("lookup_type",'skills')->where("code",$value)->where("is_active",1)->first();
                    if(!empty($getinfo)){
                        return false;
                    }else {
                        return true;
                ...
                $validator = Validator::make(
                    ...
                    array(
                        "code.required"        => trans("messages.The skills field is required"),
                        "code.unique_skills"   => trans("messages.The skills must be unique"),
                    )
                );    
            }else{
                $validator = Validator::make(
                    ...
                    array(
                        'code.required'     => trans('messages.The code field is required.'),
                    ),
                );
            }
            if ($validator->fails()) {
    ...
    public function changeStatus($modelId = 0, $status = 0){
        ...
        if ($status == 1) {
            $statusMessage   =   trans("messages.".ucfirst($type). " has been deactivated successfully");
        } else {
            $statusMessage   =  trans("messages.".ucfirst($type).  " has been activated successfully");
        }
