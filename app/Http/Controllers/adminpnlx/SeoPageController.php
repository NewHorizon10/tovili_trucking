...
function saveDoc(Request $request){
   ...
   if (!empty($thisData)) {
	$validator = Validator::make(
		...
		array(
			"page_id.required"				=>	trans("messages.This field is required"),
			"title.required"				=>	trans("messages.This field is required"),				
			"page_name.required"			=>	trans("messages.This field is required"),
		)
	);
    if($validator->fails()){
        ...
        $seo->meta_chronicles   	= $request->input('meta_chronicles');
		...
		if($request->hasFile('og_image')){
			...
			DB::commit();
			Session::flash('flash_notice', ucfirst(trans("messages.admin_Seo_page_has_been_added_successfully"))); 
...
$docs				=	SeoPage::find($Id);
	...
	if ($validator->fails()){	
		...
		$Seo_response		=	SeoPage::where('id', $Id)->update(array(
            ...
            'meta_chronicles' 	=>  $request->input('meta_chronicles'),
			'og_image' 			=>  $og_image,
		...
		if(!$Seo_response) {
			...
			DB::commit();
			Session::flash('flash_notice',  ucfirst(trans("messages.admin_Seo_page_has_been_updated_successfully"))); 
...
public function updateDocStatus($Id = 0, $Status = 0){
		...
		$model->save(); */
		if($Status == 0	){
			$statusMessage	=	ucfirst(trans("messages.admin_Seo_page_deactivated_successfully"));
		}else{
			$statusMessage	=	ucfirst(trans("messages.admin_Seo_page_activated_successfully"));
		}
...
public function deletePage($modelId, Request $request){
	...
	$ids	= base64_decode($modelId);
      ...
      Session::flash('flash_notice', ucfirst(trans("messages.admin_Seo_page_has_been_removed_successfully")));
	  return Redirect::to('adminpnlx/seo-page-manager');
	}// end deleteSeoPage()
/**
