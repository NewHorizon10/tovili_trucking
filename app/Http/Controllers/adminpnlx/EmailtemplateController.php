...
class EmailtemplateController extends Controller
{
    ...
    public function edit($enmaiman)
    {
        ...
        if (!empty($enmaiman)) {
            ...
            $language_code = Config('constants.DEFAULT_LANGUAGE.LANGUAGE_CODE');
            $languages = Language::where('is_active', 1)->get();
            return  View("admin.$this->model.edit", compact('languages', 'language_code', 'multiLanguage', 'emailTemplate', 'optionsvalue'));
        }else{
            return Redirect()->route($this->model . ".index");
        }
    }
    ...
    public function update(Request $request, $enmaiman)
    {
        ...
        if ($validator->fails()) {
            ...
            if (!empty($thisData)) {
                ...
                Session()->flash('flash_notice', ucfirst(trans("messages.admin_Email_Template_updated_successfully")));
                return Redirect()->route("$this->model.index");
            }
        }
    }
    ...
    public function emailEnable(Request $request){
        $checkedValue               = $request->checkedValue;
        $emailTemplate              = EmailTemplate::find($request->emailId);
        if($checkedValue == '1'){
            $emailTemplate->mail_enable = 1;
        }else{
            $emailTemplate->mail_enable = 0;
        }
